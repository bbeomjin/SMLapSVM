...
void alpha_update(double* warmalpha_ij, double* warmalpha_yi, double* my, double* yyi, double* xinner, double* lam, double* wweight, int* nnobs, double* nnobsdouble, 
				  int* kk, double* kkdouble, double* erci, double* ggamma, int* ytrain, double* eepsilon, double* outalpha_ij, int* maxiter)
{
	    ...
	    for (j=0;j<k;j++) {
			if (ytrain[i]==(j+1)) {	
				...
				if (temp<0) {
					...
					if (temp>(wweight[i]*gamma)) {
						temp2=(wweight[i]*gamma);
					} else {
						temp2=temp;
					}
				} 
...
/*--------------if equal or not-----------------*/
			if (ytrain[i]!=(j+1)) {	
				...
				if (temp<0) {
					...
					if (temp>(wweight[i]*(1-gamma))) {
						temp2=(wweight[i]*(1-gamma));
					} else {
						temp2=temp;
					}
				} 
...
/*check difference and compare to epsilon*/
	...
	for (i=0;i<(nobs*k);i++) {
		diff+=fabs(alpha_ij[i]-alphacheck[i]);
	}
