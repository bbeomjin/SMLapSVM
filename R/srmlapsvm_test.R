srmlapsvm2 = function(x = NULL, y, ux = NULL, gamma = 0.5, valid_x = NULL, valid_y = NULL, nfolds = 5,
                     ...
                     scale = FALSE, criterion = c("0-1", "loss"), isCombined = TRUE, nCores = 1, verbose = 0, ...)
{
  ...
  thetastep_fun = switch(fun_ind,
                         thetastep.smlapsvm,
                         thetastep.srmlapsvm2)
  
  cat("Fit c-step \n")
  # cstep_fit = cstep.srmlapsvm2(x = x, y = y, ux = ux, gamma = gamma, valid_x = valid_x, valid_y = valid_y, nfolds = nfolds,
  ...
  # opt_cstep_fit = cstep.srmlapsvm2(x = x, y = y, ux = ux, gamma = gamma, valid_x = valid_x, valid_y = valid_y, nfolds = nfolds,
  #                       lambda_seq = lambda_seq, lambda_I_seq = lambda_I_seq, theta = thetastep_fit$opt_theta,
  #                       adjacency_k = adjacency_k, normalized = normalized, weightType = weightType,
  #                       kernel = kernel, kparam = kparam, scale = scale, criterion = criterion, optModel = TRUE, nCores = nCores, ...)
...
cstep.srmlapsvm2 = function(x, y, ux = NULL, gamma = 0.5, valid_x = NULL, valid_y = NULL, nfolds = 5,
                           ...
                           scale = FALSE, criterion = c("0-1", "loss"), optModel = FALSE, nCores = 1, ...)
{
  ...
  if (!is.null(valid_x) & !is.null(valid_y)) {
    ...
    fold_err = mclapply(1:nrow(params),
                        function(j) {
                          error = try({
                            msvm_fit = srmlapsvm_compact2(anova_K = anova_K, L = L, theta = theta, y = y, gamma = gamma,
    ...
    for (i in 1:nfolds) {
      ...
      fold_err = mclapply(1:nrow(params),
                          function(j) {
                            error = try({
                              msvm_fit = srmlapsvm_compact2(anova_K = subanova_K, L = L_train, theta = theta, y = y_train, gamma = gamma,
                                                           lambda = params$lambda[j], lambda_I = params$lambda_I[j], ...)
                            })
  ...
  if (optModel) {
    anova_K = make_anovaKernel(rx, rx, kernel = kernel, kparam = kparam)
    opt_model = srmlapsvm_compact2(anova_K = anova_K, L = L, theta = theta, y = y, gamma = gamma,
...
thetastep.srmlapsvm2 = function(object, lambda_theta_seq = 2^{seq(-10, 10, length.out = 100)},
                               isCombined = TRUE, optModel = FALSE, nCores = 1, ...)
{
  ...
  if (is.null(object$opt_model)) {
    opt_model = srmlapsvm_compact2(anova_K = anova_K, L = L, theta = theta, y = y, gamma = gamma, lambda = lambda, lambda_I = lambda_I, ...)
  } else {
  ...
  if (!is.null(valid_x) & !is.null(valid_y)) {
    ...
    fold_err = mclapply(1:length(lambda_theta_seq),
                        function(j) {
                          error = try({
                            theta = find_theta.srmlapsvm2(y = y, gamma = gamma, anova_kernel = anova_K, L = L,
                            ...
                            if (isCombined) {
                              ...
                              init_model = srmlapsvm_compact2(anova_K = anova_K, L = L, theta = theta, y = y, gamma = gamma,
    ...
    theta_seq_list = mclapply(1:length(lambda_theta_seq),
                              function(j) {
                                error = try({
                                  theta = find_theta.srmlapsvm2(y = y, anova_kernel = anova_K, L = L, cmat = opt_model$cmat, c0vec = opt_model$c0vec,
    ...
    for (i in 1:nfolds) {
      ...
      init_model = srmlapsvm_compact2(anova_K = subanova_K, L = L_train, theta = theta, y = y_train,
      ...
      fold_err = mclapply(1:length(lambda_theta_seq),
                          function(j) {
                            # error = try({
                            #   theta = find_theta.srmlapsvm2(y = y_train, anova_kernel = subanova_K, L = L_train, cmat = cmat, c0vec = c0vec,
                            #                                 gamma = gamma, lambda = lambda, lambda_I = lambda_I, lambda_theta = lambda_theta_seq[j], ...)
                            #
                            #   if (isCombined) {
                            #     init_model = srmlapsvm_compact2(anova_K = subanova_K, L = L_train, theta = theta, y = y_train,
  ...
  if (optModel) {
    ...
    opt_model = srmlapsvm_compact2(anova_K = anova_K, L = L, theta = opt_theta, y = y, gamma = gamma, lambda = lambda, lambda_I = lambda_I, ...)
    out$opt_model = opt_model
  }
  class(out) = "srmlapsvm"
  return(out)
...
find_theta.srmlapsvm2 = function(y, gamma, anova_kernel, L, cmat, c0vec, lambda, lambda_I, lambda_theta = 1,
...
srmlapsvm_compact2 = function(anova_K, L, theta, y, gamma = 0.5, lambda, lambda_I,
                             eig_tol_D = 0, eig_tol_I = .Machine$double.eps, epsilon_D = 1e-8, epsilon_I = 1e-12)
{
  out = list()
  ...
  diag(K_KLK) = diag(K_KLK) + n * max(abs(K_KLK)) * epsilon_I
