rmlapsvm_compact = function(K, L, y, gamma = 0.5, lambda, lambda_I, epsilon = 1e-5, epsilon_C = 1e-5)
{
  out = list()
  ...
  #       bvec[n_class + qp_dim + (j - 1) * n_l + i] = bvec[n_class + qp_dim + (j - 1) * n_l + i] - epsilon_C 
  #     }
  #   }
  # }
...
rmlapsvm = function(x = NULL, y = NULL, ux = NULL, gamma = 0.5, lambda, lambda_I, kernel, kparam, scale = FALSE, 
                    adjacency_k = 6, normalized = TRUE, weight = NULL, weightType = "Binary", epsilon = 1e-5, epsilon_C = 1e-6)
{
  out = list()
  n_l = NROW(x)
  n_u = NROW(ux)
  ...
  solutions = rmlapsvm_compact(K = K, L = L, y = y, gamma = gamma, lambda = lambda, lambda_I = lambda_I, 
                               epsilon = epsilon, epsilon_C = epsilon_C)
